...
impl LocalPreimageStorage {
    ...
    pub async fn new(config: PreimageStorageConfig) -> PreimageStorageResult<Self> {
        let base_path = config.local_path.ok_or_else(|| {
            PreimageStorageError::InvalidConfig(
                "local_path is required for local storage".to_string(),
            )
        })?;
        ...
        Ok(Self { base_path, batch_size: config.batch_size })
    }
    ...
    async fn read_preimage_file(
        ...
        if buffer.len() < 32 {
            return Err(PreimageStorageError::Storage("Invalid file format: too short".to_string()));
        }

        let stored_hash = B256::from_slice(&buffer[..32]);
        if stored_hash != *hash {
            return Err(PreimageStorageError::Storage("Hash mismatch in stored file".to_string()));
        }
...
impl PreimageStorage for LocalPreimageStorage {
    ...
    async fn get_statistics(&self) -> PreimageStorageResult<StorageStatistics> {
        ...
        Ok(StorageStatistics::new(total_preimages, total_size_bytes, "Local".to_string()))
    }
}
